...
import {
	ReqMemberCreateInterface,
	ResMemberCreateInterface,
} from '@/service/member/interface/memberCreate.interface';
	...
	setup() {
		...
		async function onRegistUser() {
			...
			if (!pwReg.test(user.userPw)) {
				alert(
					'비밀번호는 문자, 숫자를 1개 이상 포함한 5자리 이상입니다.'
				);
				return false;
			}
			if (!user.userName) {
